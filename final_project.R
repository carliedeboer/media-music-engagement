...
# Looks like most songs are hovering around 120-130 BPM. 
...
# 3. Tempo is around 120-130 BPM. 


#### PREDICTING "SYNCABILITY" ####

# Based on the above, I'm going to create a Linear regression model to be able to predict if a song is "syncable" (works well for TV/film).

# I'll now add additonal data to our dataframe, denoting songs binary as "syncable" (TRUE) and "not syncable" (FALSE).

additional_data <- (read.csv("/Users/carlief/Desktop/additional_data.csv"))
factor(additional_data$time_signature)
additional_data$key <- as.character(additional_data$key)
additional_data$key <- revalue(additional_data$key, c("0" = "C", "1" = "C♯,D♭", "2" = "D", "3" = 
                                                                                        "D♯,E♭", "4" = "E", "5" =  "F", "6" = "F♯,G♭","7" = "G","8" = "G♯,A♭","9" = "A","10" = "A♯,B♭","11" = "B"))ad
aditional_data$mode <- as.logical(additional_data$mode)


syncable_data <- rbind(tunefind_top_100_audio_features, additional_data)



# WRITE CODE TO VALUE BINARY SYNCABLE VALUES BASED ON FINDINGS ABOVE.

if(syncable_data$energy >=0 <=1 && 
   syncable_data$valence >=0 <=1 && 
   syncable_data$danceability >=0 <=1 &&
   syncable_data$key == "C, G, F" &&
   syncable_data$tempo >=120 <=130)
print("TRUE")
...
n <- nrow(tunefind_top_100_audio_features)
ntrain <- round(n*0.6)
set.seed(56)
tindex <- sample(n, ntrain)

train_tunefind_top_100_audio_features <- tunefind_top_100_audio_features[tindex,]
test_tunefind_top_100_audio_features <- tunefind_top_100_audio_features[-tindex,]

lm2 <- lm(tempo~., data=train_tunefind_top_100_audio_features)
summary(lm2)




# We need a NO set of data (75-100). 
